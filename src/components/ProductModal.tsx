...
interface IModal extends IProductCard {
...
const ModalProduct: FC<IModal> = ({
  setShowModal,
  skuField = "SKU Produk",
  firstField = "Nama Produk",
  secondField = "Deskripsi Produk",
  thirdField = "Foto Produk",
  submitTitle = "Perbarui Produk",
  priceText = "Harga Produk",
  stockText = "Jumlah Persediaan",
  imageUrl,
  name,
  description,
  price,
  ...
  const newPrice = Number(String(price).replace("Rp ", "").split(".").join(""));
  ...
  const productControl = (key: string, value: string | number) => {
    setProduct((oldData) => {
      oldData[`${key}`] = value;
      return { ...oldData };
    });
  ...
  return (
    <div className="flex justify-center items-center bg-zinc-300/50 bg-opacity-100 overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
      <div className="relative my-6 pt-6 mx-auto w-1/2">
        <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          ...
          <div className="relative p-6 py-3 flex-auto">
            <form className="rounded px-4 pt-3 pb-4 w-full">
              <label className="block text-black text-sm font-bold mb-1">
                {skuField}
              </label>
              <input
                value={product.sku}
                onChange={(e) => productControl("sku", e.target.value)}
              ...
              <input
                value={product.name}
                onChange={(e) => productControl("name", e.target.value)}
                className="mb-4 shadow appearance-none border rounded w-full py-2 px-1 text-black"
              />
              <label className="block text-black text-sm font-bold mb-1">
                {secondField}
              ...
              <textarea
                ...
                onChange={(e) => productControl("description", e.target.value)}
                className="mb-4 shadow appearance-none border rounded w-full py-2 px-1 text-black"
              />
              <label className="block text-black text-sm font-bold mb-1">
                {thirdField}
              ...
              <input
                ...
                value={product.price}
                min={0}
                onChange={(e) => productControl("price", e.target.value)}
                className="mb-4 shadow appearance-none border rounded w-full py-2 px-1 text-black"
              />
              <label className="block text-black text-sm font-bold mb-1">
                {stockText}
              ...
              <input
                ...
                value={stocks}
                min={0}
                onChange={(e) => productControl("stocks", e.target.value)}
                className="mb-4 shadow appearance-none border rounded w-full py-2 px-1 text-black"
              />
            </form>
          </div>
